-- ADD PRODUCT FEATURES TO PRODUCTS TABLE (CORRECTED VERSION)
...
WHERE features IS NULL OR (features IS NOT NULL AND array_length(features, 1) IS NULL);
...
SELECT 
  ...
  features,
  CASE 
    WHEN features IS NULL THEN 0
    ELSE array_length(features, 1)
