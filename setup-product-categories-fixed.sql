-- Setup Product Categories Table and Data - FIXED VERSION
...
-- Step 2: Add self-referencing foreign key after table creation
...
-- Step 4: Create a trigger function to update the updated_at timestamp
...
-- Step 5: Create the trigger (only after table and function exist)
...
-- Step 6: Insert the existing categories from the system
...
-- Step 7: Add a category_id column to the products table if it doesn't exist
...
-- Step 8: Add foreign key constraint to products table
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Step 9: Update existing products to use the new category system
...
-- Step 11: Show the categories that were created
SELECT 
    name,
    description,
    display_order,
...
-- Step 12: Show products and their category mappings
SELECT 
