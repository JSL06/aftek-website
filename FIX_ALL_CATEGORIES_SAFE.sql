-- =====================================================
-- SAFE FIX FOR AFTEK WEBSITE CATEGORIES
-- =====================================================
-- This script safely creates missing tables and adds missing categories
-- It handles existing policies and tables without errors
-- Run this in your Supabase SQL editor to fix ALL the 400 errors

-- =====================================================
-- STEP 1: CREATE ALL MISSING TABLES (SAFELY)
-- =====================================================
...
-- =====================================================
-- STEP 2: INSERT ALL PRODUCT CATEGORIES (SAFELY)
-- =====================================================
...
-- =====================================================
-- STEP 3: ADD ALL CATEGORY TRANSLATIONS (SAFELY)
-- =====================================================
...
-- =====================================================
-- STEP 4: ADD FEATURES FOR ALL CATEGORIES (SAFELY)
-- =====================================================
...
-- =====================================================
-- STEP 5: ADD FEATURE TRANSLATIONS (SAFELY)
-- =====================================================
...
-- =====================================================
-- STEP 6: CREATE INDEXES AND SECURITY (SAFELY)
-- =====================================================
...
BEGIN
  ...
  -- Policy for master_features
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'master_features' 
    AND policyname = 'Allow public read access to master_features'
  ) THEN
    CREATE POLICY "Allow public read access to master_features" ON master_features
      FOR SELECT USING (true);
  END IF;

  -- Policy for feature_translations
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'feature_translations' 
    AND policyname = 'Allow public read access to feature_translations'
  ) THEN
