-- =====================================================
-- AFTEK WEBSITE - COMPLETE DATABASE SETUP SCRIPT (UUID FIXED)
-- =====================================================
-- This script handles UUID primary keys correctly
...
-- =====================================================
-- 2. CREATE PRODUCTS TABLE (WITH UUID)
-- =====================================================
...
-- =====================================================
-- 6. CREATE FEATURED_PRODUCTS TABLE (WITH UUID REFERENCE)
-- =====================================================
...
DO $$ 
BEGIN
    -- Check if products table uses UUID or SERIAL and handle accordingly
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'id' AND data_type = 'uuid') THEN
        -- Products table already uses UUID, just add missing columns
        NULL;
    ELSIF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'products' AND column_name = 'id' AND data_type = 'integer') THEN
        -- Products table uses integer, need to convert or handle differently
        -- For safety, we'll just add missing columns to existing structure
        NULL;
    END IF;
...
-- =====================================================
-- 8. FIX FEATURED_PRODUCTS TABLE FOR EXISTING UUID PRODUCTS
-- =====================================================

DO $$
BEGIN
    -- Drop and recreate featured_products table if it exists with wrong foreign key type
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'featured_products') THEN
        -- Check if the foreign key constraint exists and has wrong type
        IF EXISTS (
            ...
            -- Drop the existing featured_products table and recreate with correct UUID type
...
-- =====================================================
-- 9. UPDATE NULL VALUES WITH PROPER DEFAULTS
-- =====================================================
...
-- =====================================================
-- 11. INSERT SAMPLE WEBSITE TEXTS
-- =====================================================
...
-- =====================================================
-- 12. INSERT SAMPLE PRODUCTS (WITH UUID)
-- =====================================================
...
-- Show the data types of ID columns to verify UUID usage
SELECT 
    table_name,
    column_name,
    data_type,
...
-- =====================================================
-- SCRIPT COMPLETE - UUID COMPATIBLE!
-- ===================================================== 